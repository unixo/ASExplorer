...
public class CommandLocator
{
    ...
    {
        HashMap<String, CommandInterface> commands = new HashMap<String, CommandInterface>();
        Class[] commandClasses;

        try {
            commandClasses = CommandLocator.getClasses("asexplorer.command");

            for (Class<CommandInterface> c : commandClasses) {
                CommandInterface ci = c.newInstance();
                commands.put(ci.getCommandName(), ci);
            }
        } catch (Exception ex) {
            return null;
        }
    ...
    private static List<Class> findClasses(File directory, String packageName) throws ClassNotFoundException, IOException
    {
        ...
        for (File file : files) {
            ...
            if (file.isDirectory()) {
                ...
                if (CommandLocator.isValidCommand(c)) {
                    classes.add(c);
                }
            }
        }
    ...
    private static boolean isValidCommand(Class aClass)
    {
        Class[] interfaces = aClass.getInterfaces();

        for (Class c : interfaces) {
            if (c.equals(CommandInterface.class)) {
                return true;
            }
        }
